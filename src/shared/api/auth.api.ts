 ...
 */
export const loginUser = async (data: LoginRequest): Promise<ApiResponse<User>> => {
  if (data.__mock) {
    // Mock режим
 ...
 * Иначе отправляет реальный запрос на сервер
 */
export const registerUser = async (data: RegisterRequest): Promise<ApiResponse<{ message: string }>> => {
  if (data.__mock) {
    // Mock режим
