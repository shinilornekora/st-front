<template>
  <div :class="$style.statCard">
    <div :class="$style.statHeader">
      ...
      <select :class="$style.periodSelect" v-model="localPeriod" @change="handlePeriodChange">
...
const productsSoldDatasets = computed(() => {
  ...
  return props.selectedProductIds.map((productId, index) => {
    const product = props.productsData.find(p => p.id === productId);
    if (!product) return null;
    
    return {
      label: product.name,
      data: generateProductChartData(productId, localPeriod.value),
      color: props.productColors[index % props.productColors.length],
    };
